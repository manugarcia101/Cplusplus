// Practica8MallasRegulares.cpp: define el punto de entrada de la aplicación de consola.
//
...
int main(int argc, const char * argv[]) {
	...
	for (unsigned int i = 0; i < vlon.size(); i++) {
		int posX = static_cast<int>((vlat[i] - minlat) * pixelPorGradoX);
		int posY = static_cast<int>(ncol - 1 - ((vlon[i] - minlon) * pixelPorGradoY));
	...
	img.pintarRecuadro(static_cast<int>((lat1 - minlat) * pixelPorGradoX), static_cast<int>(ncol - 1 - ((lon1 - minlon) * pixelPorGradoY)),
		static_cast<int>((lat2 - minlat) * pixelPorGradoX), static_cast<int>(ncol - 1 - ((lon2 - minlon) * pixelPorGradoY)), 255, 0, 0);
